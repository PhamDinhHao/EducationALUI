...
const SearchRecipientModal: React.FC<Props> = ({
  isOpen,
  onClose,
  isSituation = true,
  onFetch,
  ...
  const handleSubmit = useCallback(() => {
    ...
    Object.keys(processedFilter.criteria).forEach(groupKey => {
      ...
      group.forEach(item => {
        if (item.field === 'numberOfError' && item.searchValue) {
        ...
        item.field = _.snakeCase(item.field)
        item.searchType = _.snakeCase(item.searchType)
      });
    });
    onSetQueryParams(processedFilter)
    onFetch(processedFilter)
    onClose()
  }, [filter]);
...
export default SearchRecipientModal
