...
const MENU_ITEMS: MenuItem[] = [
  ...
  },
  {
    ...
    label: 'Giáo viên',
    children: [
      { key: 'teacher-plan', label: <Link to={PagePath.BUILD_STRUCTURE}>Xây dựng đề</Link> },
      { key: 'teacher-exam', label: <Link to={PagePath.BUILD_LESSON}>Xây dựng giáo án</Link> },
      { key: 'teacher-mark', label: <Link to={PagePath.PERSONAL_PLAN}>Kế hoạch cá nhân & Sáng kiến kinh nghiệm</Link> },
      { key: 'teacher-report', label: <Link to={PagePath.TEACHER_ASSISTANT}>Trợ lý giáo viên</Link> }
    ]
  },
  {
    key: 'student',
