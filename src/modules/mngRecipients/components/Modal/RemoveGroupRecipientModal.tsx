...
import {
  RegistrationRecipientSchemaGroup,
  TRegistrationRecipientGroup
} from '@/modules/mngRecipients/core/config/form/bulk-add-group'
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { bulkRemoveGroup } from '@/modules/mngRecipients/services/recipient.service'
...
const RemoveGroupRecipientModal: React.FC<Props> = ({
  isOpen,
  onClose,
  ids,
  onResetBulkModal,
  ...
  const handleConfirm = async () => {
    ...
    return await bulkRemoveGroup(transformedValue)
  }
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      open={isOpen}
      title={<div className='text-center text-xl font-bold'>リストから除外</div>}
      footer={[
        <Button key='cancel' onClick={onClose}>
          キャンセル
        </Button>,
        <Button key='submit' type='primary' disabled={checkedIds.length === 0} danger onClick={handleSubmit(onSubmitForm)}>
          登録
        </Button>
      ]}
    >
...
export default RemoveGroupRecipientModal
